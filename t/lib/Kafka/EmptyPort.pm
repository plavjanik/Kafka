package Kafka::EmptyPort;
use strict;
use warnings;
use base qw/Exporter/;
use Time::HiRes ();
our @EXPORT = qw/ can_bind find_empty_port check_port wait_port /;
...
sub check_port {
    ...
    # for UDP, we do a local port check, like find_empty_port does
    my $sock;
    if ($ENV{IPV6}) {
        $sock = ($proto eq 'tcp') ?
            IO::Socket::IP->new(
...
Kafka::EmptyPort - find a free TCP/UDP port

=head1 SYNOPSIS

    use Kafka::EmptyPort qw(find_empty_port check_port);

    # get a socket listening on a random free port
    my $socket = listen_socket();

    # get a random free port
    my $port = find_empty_port();
...
Kafka::EmptyPort helps finding an empty TCP/UDP port. It is a modified version of Net::EmptyPort that works
well on Perl 5.010 and uses only IO::Socket::INET for IPv4.
...
=item C<< find_empty_port() >>

=item C<< find_empty_port(\%args) >>

=item C<< find_empty_port($port) >>

=item C<< find_empty_port($port, $proto) >>
...
the second parameter:
    ...
    my $port = find_empty_port(1024, 'udp');
    # use 49152..65535 range
    my $port = find_empty_port(undef, 'udp');
